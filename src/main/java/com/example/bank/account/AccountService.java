...
public class AccountService {
    ...
    public AccountResponse.TransferDTO 계좌이체(AccountRequest.TransferDTO reqDTO, SessionUser sessionUser) {
        ...
        // 2. 0원 이하의 금액은 들어오면 안됨
        if (reqDTO.getAmount() <= 0) {
            throw new Exception400("0원 이하 금액은 이체할 수 없습니다.");
        }

        // 핵심 로직 update 2번, insert 1번
        // 3. 입출금 계좌 조회
        ...
        if (!senderAccount.getUser().getId().equals(sessionUser.getId())) {
            throw new Exception403("계좌이체 권한이 없습니다.");
        }

        // 5. 출금 계좌 비번 확인
        if (!senderAccount.getPassword().equals(receiverAccount.getPassword())) {
        ...
        // 7. 계좌 상태 변경
//        Long currentSenderBalance = senderAccount.getBalance();
//        senderAccount.setBalance(currentSenderBalance - reqDTO.getAmount());
        senderAccount.withdraw(reqDTO.getAmount());
//        Long currentReceiverBalance = receiverAccount.getBalance();
//        receiverAccount.setBalance(currentReceiverBalance + reqDTO.getAmount());
